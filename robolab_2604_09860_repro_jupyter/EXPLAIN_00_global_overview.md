...
| 感觉精讲还是不够深 | 精讲 17 | 它把 RoboLab 当成评测编译链，按输入、状态、输出、失败边界和证据落点重读 task/scene/env/policy/event |

> [!TIP]
> **这版精讲0的核心变化**：它不是替代后面的 16 个精讲，而是告诉你后面的精讲分别在回答哪类问题，以及复现实验里应该用哪些证据去验证这些问题。
...
| 深水机制线 | 把所有模块按状态机和调试路径串起来 | 17 |

> [!WARNING]
> **最容易出错的地方**：四条线任何一条断了，最终结果都不能直接解释成“模型能力强/弱”。例如资产缺失是生成线问题，任务谓词错是任务线问题，server timeout 是运行线问题，dashboard 没读到 HDF5 是证据线问题。
...
| 深水机制 | 17 | task contract、typed predicates、env binding、policy rollout、WorldState/EventTracker、扰动和 baseline 边界如何串成状态机 | 形成可调试心智模型 |
...
> [!NOTE]
> **读法建议**：以后如果要继续加精讲，不要只加正文解释。先在 `EXPLAIN_SOURCE_QUESTION_INDEX.md` 里补“问题来源”，再在本精讲0里判断它属于生成线、任务线、运行线、证据线还是评价线。
