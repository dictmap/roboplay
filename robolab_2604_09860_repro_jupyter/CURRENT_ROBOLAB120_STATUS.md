...
Cosmos policy prefetch is running through `HF_ENDPOINT=https://hf-mirror.com`; logs are under `/data/light/roboplay_models/cosmos/logs/`. Reason2-2B and Reason2-8B are gated for the current account/token and need Hugging Face authorization before full download.
